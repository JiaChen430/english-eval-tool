  ...
  async function fetchVideos() {
    ...
    try {
      const res = await fetch('/api/videos', {
        ...
        body: JSON.stringify({ error, corrected, explanation }),
      });
  ...
  return (
    <div className="mb-4">
      <button
        onClick={fetchVideos}
        disabled={loading}
        className="flex items-center gap-2 px-3 py-1.5 text-sm bg-red-50 text-red-700 border border-red-200 rounded-lg hover:bg-red-100 transition-colors disabled:opacity-50"
      >
        <span>📺</span>
        <span>{loading ? 'Loading...' : videos.length > 0 ? 'Hide Videos' : 'Watch Videos'}</span>
      </button>
          ...
          {videos.map((vid, idx) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-600 mt-1 line-clamp-2">{vid.reason}</p>
              </div>
            </a>
          ))}
        </div>
        ...
        {exercises.map((item, idx) => {
          ...
          return (
            ...
            >
              ...
              <VideoButton
                error={item.error.original}
                corrected={item.error.corrected}
                explanation={item.error.explanation}
              />
